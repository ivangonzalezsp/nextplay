...
async function main() {
    ...
    try {
        ...
        if (!before)
            throw new Error(
                'No hay una biblioteca sincronizada que actualizar.',
            );
        ...
        try {
            ...
            if (!saved || identity(saved) !== beforeIdentity)
                throw new Error(
                    'La copia de seguridad no coincide con la biblioteca.',
                );
        } finally {
            backup.close();
        }
        ...
        if (!pending.length) {
            console.log(
                `steam-tags: nada que actualizar; backup=${backupPath}`,
            );
            return;
        }
        ...
        try {
            for (const appId of pending) {
                ...
                try {
                    const tags = await client.fetchGameTags(
                        appId,
                        dictionaries,
                    );
                    writes.push({
