...
void test('family data separates ownership, excludes unavailable apps and checks the playtime owner', () => {
  ...
  assert.deepEqual(
    parsed.games.map((g) => [g.appId, g.owned, g.shared, g.playtimeMinutes]),
    [
      [1, true, false, 120],
      [2, false, true, 15],
    ],
      ...
      familyGames(
        {
          response: {
            owner_steamid: viewer,
            apps: [response.response.apps[0], response.response.apps[0]],
          },
        },
        viewer,
      ),
    ...
    globalThis.fetch = async (input) => {
            ...
            response:
              mode === 'left'
                ...
                : {
                    ...
                    family_group: {
                      name: 'Familia',
                      members: [{ steamid: viewer }, { steamid: lender }],
                    },
                  },
          });
      ...
      if (url.pathname.includes('GetOwnedGames')) {
        ...
        return Response.json({
          response: {
            game_count: 1,
            games: [{ appid: 1, name: 'Juego propio', playtime_forever: 600 }],
          },
        });
      }
