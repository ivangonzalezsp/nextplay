...
void test('recent suggestions lose priority without exclusions; explicit requests and hard filters prevail', () => {
  ...
  assert.equal(
    selectCandidates(state, [], DEFAULT_FILTERS, 'Quiero Candidate 1')[0].appId,
    1,
  );
  state.preferences[1] = { favorite: false, status: 'ignored' };
  assert.ok(
    !selectCandidates(state, [], DEFAULT_FILTERS, 'Quiero Candidate 1').some(
      (g) => g.appId === 1,
    ),
  );
  assert.equal(
    selectCandidates(state, [], { ...DEFAULT_FILTERS, mode: 'next', hours: 5 })
      .length,
    0,
  );
  ...
  const data = JSON.parse(
    buildPrompt(state, games, DEFAULT_FILTERS, '').split('DATOS_JSON:\n')[1],
  );
  assert.deepEqual(
