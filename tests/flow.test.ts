...
void test('Steam URL and preferences are validated at the boundary', () => {
    ...
    assert.throws(() =>
        parsePreference({ favorite: 'yes', status: 'pending' }),
    );
    assert.throws(() =>
        parsePreference({ favorite: true, status: '__proto__' }),
    );
    ...
    const fetcher: typeof fetch = async (input) => {
        ...
        if (u.pathname.includes('GetPlayerSummaries'))
            return Response.json({
                response: {
                    players: [
                        { steamid: profile.steamId, personaname: 'Jugador' },
                    ],
                },
            });
...
void test('exclusions win, ownership is authoritative and favorites outrank hours', () => {
    ...
    const replay = selectCandidates(s, [], {
        ...DEFAULT_FILTERS,
        replay: true,
    });
...
void test('Codex output rejects invented IDs, duplicates, wrong ownership and missing reasons', () => {
    ...
    assert.equal(
        validatePicks(valid, candidates).owned[0].game.name,
        'Juego 1',
    );
    for (const wrong of [
...
void test('local recommendations use taste weights, mode signals and hard filters without inventing session lengths', () => {
    ...
    assert.equal(
        recommendLocally(s, { ...DEFAULT_FILTERS, mode: 'next', hours: 5 })
            .owned.length,
        0,
    );
    assert.equal(
        recommendLocally(s, { ...DEFAULT_FILTERS, gameMode: 'coop' }).owned
            .length,
        0,
    );
    assert.equal(
        recommendLocally(s, { ...DEFAULT_FILTERS, genre: 'Racing' }).owned
            .length,
        0,
    );
...
void test('local API works without Codex or network and preserves every search across resets, feedback and reloads', async () => {
    ...
    try {
        ...
        assert.deepEqual(
            migrated.history?.[0].result,
            s.conversation[0].result,
        );
        const call = async (
            path: string,
            payload: object,
        ...
        for (const date of ['2026-02-30', '2099-01-01'])
            assert.equal(
                (await request('play-history', { index: 0, date }, 'PATCH'))
                    .status,
                400,
            );
        assert.equal(
            dateInputValue((await readState()).playHistory![0].at),
            '2026-05-15',
        );
