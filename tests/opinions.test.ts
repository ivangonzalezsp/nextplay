...
void test('explicit opinions persist, override hours, reach both engines and can be removed safely', async () => {
    ...
    assert(
        weight() < 0,
        'explicit dislike overrides long playtime and favorite',
    );
    assert(
    ...
    try {
        ...
        assert.equal(
            (await update('loved', '  Buenas ideas, pero demasiado largo.  '))
                .status,
            200,
        );
        ...
        assert(
            payload.tasteProfile.find(
                (a: { id: string }) => a.id === 'automation',
            ).inferred > 0,
        );
