  ...
  try {
    ...
    globalThis.fetch = async (url, init) => {
      ...
      if (path.startsWith('/v4/')) {
        ...
        queries.push(query);
        if (path === '/v4/games') return Response.json(missing ? [] : [raw]);
        ...
        if (path === '/v4/game_time_to_beats')
          return Response.json([{ game_id: 132, count: 10, hastily: 72000 }]);
      }
      if (path.includes('GetPlayerSummaries'))
        return Response.json({
          response: { players: [{ steamid: viewer, personaname: 'Test' }] },
        });
      if (path.includes('GetOwnedGames'))
        return Response.json({
          response: {
            game_count: 1,
            games: [
              { appid: 132, name: steamGame.name, playtime_forever: 120 },
            ],
          },
        });
    ...
    assert.equal(gameUrl(steamGame), 'https://store.steampowered.com/app/132/');
    ...
    assert.equal(
      (await call('shortlist', { appId: -1, saved: true }, 'PATCH')).status,
      200,
    );
    ...
    assert.equal(
      (
        await call(
          'state',
          { appId: -1, preference: { favorite: true, status: 'completed' } },
          'PATCH',
        )
      ).status,
      200,
    ...
    assert.equal(
      familyRefreshed.state.games.find((game) => game.appId < 0)?.ownerSteamIds,
      undefined,
    );
