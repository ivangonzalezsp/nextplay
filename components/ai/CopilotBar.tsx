...
import type { Game, RecommendationEngine, CodexSettings, Snapshot } from '@/lib/model';
    ...
    <section className="hud-copilot-dock">
          ...
          <div>
            <h4 className="text-sm font-semibold text-foreground flex items-center gap-2">
              ...
              <span className="hud-copilot-pill">
                {engine === 'codex' ? `${codex.model} · ${codex.effort}` : 'Motor local'}
              </span>
            </h4>
        ...
        <div className="flex items-center gap-2">
          {conversationHistory.length > 0 && (
            ...
            >
              ...
              {historyOpen ? <ChevronUp size={12} className="ml-1" /> : <ChevronDown size={12} className="ml-1" />}
            </Button>
          )}
          ...
          {conversationHistory.map((turn, index) => (
            <div key={index} className="hud-history-turn">
              <div className="hud-history-user">
                <strong>Tú:</strong> {turn.text || 'Búsqueda inicial'}
              </div>
      ...
      {reference && (
        <div className="hud-copilot-reference-chip">
          ...
          <span>
            Buscando algo similar a: <strong>{reference.name}</strong>
          </span>
          <button
            type="button"
      ...
      >
        ...
        <Button
          type="submit"
          size="sm"
          disabled={!text.trim() || engine !== 'codex' || !canRecommend || !!busy}
      ...
      {engine !== 'codex' && (
        <div className="hud-engine-warning-banner">
          <span>Estás en modo motor local (offline). Para afinar con IA, cambia a Codex:</span>
          <Button
            variant="outline"
            size="sm"
