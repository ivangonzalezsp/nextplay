...
export function QuickVibeBar({
  ...
  filterCounts: { eligible: number; total: number; missing: number; relaxedTags?: boolean } | null;
  genres: string[];
  ...
  const activeFilterChips: { label: string; patch: Partial<Filters> }[] = [
    ...(filters.shortlistOnly
      ...
      : []),
    ...(filters.comfortZone
      ? [{ label: 'Zona de confort activa', patch: { comfortZone: false } }]
      : []),
    ...(filters.mode === 'today' && filters.sessionIntent && filters.sessionIntent !== 'any'
      ? [
          {
            label:
    ...
    ...(filters.mode === 'next' && filters.hours !== null
      ? [{ label: `Historia ≤ ${filters.hours}h`, patch: { hours: null } }]
      : []),
    ...(filters.mode === 'today' && filters.minutes !== null
      ? [{ label: `${filters.minutes} min sesión`, patch: { minutes: null } }]
      : []),
    ...(filters.minReleaseDate
      ? [{ label: `Desde ${filters.minReleaseDate}`, patch: { minReleaseDate: null } }]
      : []),
    ...
    ...(filters.gameMode
      ? [
          {
            label:
              { single: 'En solitario', coop: 'Cooperativo', multi: 'Multijugador' }[
                filters.gameMode
              ] ?? filters.gameMode,
            patch: { gameMode: '' },
          },
  ...
  const quickMoods = [
    { label: 'Relax / Desconectar', mood: 'Algo tranquilo y relajante, sin prisas ni estrés', icon: Smile },
    { label: 'Historia profunda', mood: 'Una gran historia absorbente con buena narrativa', icon: BookOpen },
    { label: 'Acción / Reto', mood: 'Acción directa, dinámico y desafiante', icon: Swords },
    { label: 'Para cooperativo', mood: 'Ideal para jugar en cooperativo', icon: Users },
  ];
  ...
  function addTagFilter() {
    ...
    const exact = steamTags.find(
      (tag) => tag.value === tagSearch.trim() || tag.label.toLocaleLowerCase() === query,
    );
    const match =
      exact ?? steamTags.find((tag) => tag.label.toLocaleLowerCase().includes(query));
    if (!match) return;
      ...
      <div className="hud-controls-header">
        <div className="hud-mode-segmented">
          <button
            ...
            onClick={() => setFilters({ ...filters, mode: 'today' })}
          >
            <Clock3 size={15} />
            <span>Para hoy</span>
          </button>
        ...
        <div className="hud-header-tools">
          ...
          <Sheet open={filtersSheetOpen} onOpenChange={setFiltersSheetOpen}>
            <SheetContent side="right" className="hud-filters-sheet sm:max-w-md">
              <SheetHeader>
                ...
                <SheetDescription className="text-xs">
                  Ajusta los criterios de selección para tu biblioteca.
                </SheetDescription>
              </SheetHeader>
              ...
              <div className="space-y-4 py-4 overflow-y-auto max-h-[calc(100vh-140px)] pr-1">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        setFilters({
                          ...filters,
                          sessionIntent: val as Filters['sessionIntent'],
                        })
                      }
                      items={[
                        { value: 'any', label: 'Cualquiera (empezar o continuar)' },
                        { value: 'continue', label: 'Continuar partida en curso o pausa' },
                        { value: 'start', label: 'Empezar un juego pendiente' },
                      ]}
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="any">Cualquiera (empezar o continuar)</SelectItem>
                        <SelectItem value="continue">Continuar partida en curso o pausa</SelectItem>
                        <SelectItem value="start">Empezar un juego pendiente</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                )}
                ...
                <div>
                  ...
                  <Select
                    value={filters.genre}
                    onValueChange={(val) => setFilters({ ...filters, genre: val ?? '' })}
                    items={[
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="">Cualquier género</SelectItem>
                      {genres.map((g) => (
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(val) => setFilters({ ...filters, gameMode: val ?? '' })}
                    items={[
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="single">En solitario</SelectItem>
                      <SelectItem value="coop">En cooperativo</SelectItem>
                ...
                <div>
                  <label htmlFor="hud-tags-input" className="text-xs font-semibold text-foreground mb-1 block">
                    Etiquetas de Steam
                  </label>
                  <div className="flex gap-2">
                    ...
                    />
                    <Button type="button" size="sm" variant="secondary" onClick={addTagFilter}>
                      Añadir
                    </Button>
                  </div>
                  <datalist id="hud-tags-suggestions">
                    {steamTags
                      .filter((t) => !selectedTagKeys.includes(t.value))
                      .slice(0, 30)
                      .map((t) => (
                  ...
                  {selectedTagKeys.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {selectedTagKeys.map((key) => (
                        <span key={key} className="hud-filter-tag">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setFilters({
                                ...filters,
                                tags: selectedTagKeys.filter((t) => t !== key),
                              })
                            }
                          >
                            <X size={12} />
                ...
                {/* Min Release Date */}
                <div>
                  <label htmlFor="hud-release-date" className="text-xs font-semibold text-foreground mb-1 block">
                ...
                <div className="space-y-2.5 pt-2 border-t border-border/50">
                  <label className="flex items-center gap-2 text-xs cursor-pointer text-foreground">
                    ...
                    />
                    <span>Sácame de mi zona de confort (propuestas audaces)</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 text-xs cursor-pointer text-foreground">
                    ...
                    />
                    <span>Incluir juegos ya terminados o abandonados</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 text-xs cursor-pointer text-foreground">
                    ...
                    />
                    <span>Solo mi lista corta ({savedGamesCount} candidatos)</span>
                  </label>
                </div>
          ...
          <div className="hud-pill-selector">
            {[
              ...
              <button
                ...
                onClick={() => setFilters({ ...filters, minutes: val })}
              >
                {label}
              </button>
            ))}
          ...
          <div className="hud-pill-selector">
            {[
              ...
              <button
                ...
                onClick={() => setFilters({ ...filters, hours: val })}
              >
                {label}
              </button>
            ))}
          ...
          {activeFilterChips.length > 0 ? (
            <div className="flex flex-wrap items-center gap-1.5">
              {activeFilterChips.map(({ label, patch }, index) => (
                <span key={index} className="hud-filter-tag active">
                  {label}
                  <button
                    type="button"
              ...
              <button
                ...
                onClick={() => setFilters(clearFilters(filters.mode))}
              >
                Limpiar
              </button>
            </div>
