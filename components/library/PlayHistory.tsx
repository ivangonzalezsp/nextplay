...
function EventLabel({ event }: { event: PlayEvent }) {
  const Icon = { started: Play, playing: Play, completed: Check, paused: Pause, abandoned: X, pending: Clock, ignored: Ban }[event.kind];
  const phase = playEventPhase(event);
  return <span className="inline-block rounded border-l-2 bg-muted px-1.5 py-1 text-xs font-medium [overflow-wrap:anywhere]" style={{ borderColor: gameColor(event.appId) }}>
...
function EventCover({ src }: { src?: string }) {
  ...
  return <div className="flex aspect-[2/3] w-full items-center justify-center overflow-hidden rounded-md bg-muted">
    {src && failedSrc !== src
      ? <img src={src} alt="" loading="lazy" className="h-full w-full object-cover" onError={() => setFailedSrc(src)} />
      : <Gamepad2 className="w-1/2 max-w-8 text-muted-foreground" aria-hidden="true" />}
  </div>;
}
...
export function PlayHistory({ state, busy, onDateChange }: { state: State; busy: boolean; onDateChange: (index: number, date: string) => Promise<void> }) {
  ...
  const covers = new Map([
    ...[...(state.history ?? []), ...state.conversation].flatMap((turn) => [...turn.result.owned, ...turn.result.discoveries].map((pick) => pick.game)),
    ...(state.shortlist ?? []), ...state.games,
  ].filter((game) => game.cover).map((game) => [game.appId, game.cover]));
  const [now] = useState(() => Date.now());
  const currentYear = new Date(now).getFullYear();
  const [year, setYear] = useState(currentYear);
  const [view, setView] = useState<'timeline' | 'calendar'>('timeline');
  const earliestYear = Math.min(currentYear, ...events.map((event) => new Date(event.at).getFullYear()));
  ...
  return <section className="space-y-6" aria-label="Mi año de juegos">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">{count('started')} juegos empezados · {count('completed')} terminados</p>
      </div>
      <div className="flex flex-wrap items-center gap-2">
        <label htmlFor="play-year" className="text-sm">Año</label>
        <select id="play-year" value={year} onChange={(event) => setYear(Number(event.target.value))} className="rounded-md border border-input bg-background px-3 py-2">
          {years.map((value) => <option key={value} value={value}>{value}</option>)}
        </select>
        <Button variant={view === 'timeline' ? 'default' : 'outline'} aria-pressed={view === 'timeline'} onClick={() => setView('timeline')}><List size={16} /> Timeline</Button>
    ...
    {view === 'timeline' ? <div className="space-y-6">
      <div className="overflow-x-auto rounded-xl border border-border bg-card p-4" role="region" aria-label={`Tramos de juego de ${year}`} tabIndex={0}>
        <div className="min-w-[640px] space-y-4">
          <div className="ml-52 grid grid-cols-12 text-xs text-muted-foreground">{months.map((month) => <span key={month}>{month.slice(0, 3)}</span>)}</div>
          {periods.map((period, index) => {
            const span = periodSpan(period, yearStart, yearEnd, now)!;
            ...
            return <div key={index} className="flex items-center gap-4">
              <a href={eventUrl(period.start)} target="_blank" rel="noreferrer" className="flex w-48 shrink-0 items-center gap-2 hover:underline">
                <span className="w-9 shrink-0"><EventCover src={covers.get(period.start.appId) ?? period.start.cover} /></span>
                <span className="text-sm">{period.start.name}</span>
              </a>
              <div className="min-w-0 flex-1">
                <p className="mb-2 text-xs text-muted-foreground">Start {dateLabel(period.start.at)} · {period.end ? `${playEventLabel(period.end)} ${dateLabel(period.end.at)}` : 'En curso'}</p>
                <div className="relative h-5 rounded bg-muted/40" aria-label={description} title={description}>
                  <span className="absolute top-2 h-1 rounded-full opacity-70" style={{ left: `${span.start / yearDays * 100}%`, width: `${(span.end - span.start + 1) / yearDays * 100}%`, backgroundColor: color }}>
                    <span className="absolute -left-0.5 -top-0.5 h-2 w-2 rounded-full" style={{ backgroundColor: color }} />
                    <span className={`absolute -right-0.5 -top-0.5 h-2 w-2 border ${period.end ? 'rounded-sm' : 'rounded-full bg-card'}`} style={{ borderColor: color, ...(period.end ? { backgroundColor: color } : {}) }} />
                  </span>
                </div>
              </div>
            </div>;
      ...
      {months.map((monthName, month) => {
        ...
        const monthEvents = selected.filter((event) => new Date(event.at).getMonth() === month);
        return <section key={month} className="min-w-0 rounded-xl border border-border bg-card p-3" aria-label={`${monthName} ${year}`}>
          <h3 className="mb-3 text-lg font-semibold capitalize">{monthName}</h3>
          <div className="grid grid-cols-7 gap-1">
            {['L', 'M', 'X', 'J', 'V', 'S', 'D'].map((day) => <span key={day} className="text-center text-xs text-muted-foreground">{day}</span>)}
          </div>
          {Array.from({ length: Math.ceil((offset + days) / 7) }, (_, week) => {
            const firstDay = week * 7 - offset + 1;
            const weekStart = new Date(year, month, Math.max(1, firstDay));
            const weekEnd = new Date(year, month, Math.min(days, firstDay + 6));
            return <div key={week} className="mb-2">
              <div className="grid grid-cols-7 gap-1">
            {Array.from({ length: 7 }, (_, column) => {
              const day = firstDay + column;
              if (day < 1 || day > days) return <div key={column} />;
              const dayEvents = monthEvents.filter((event) => new Date(event.at).getDate() === day);
              return <div key={day} className={`min-h-16 min-w-0 rounded-md border p-1 ${dayEvents.length ? 'border-primary/40 bg-primary/10' : 'border-border'}`}>
                <span className="text-xs text-muted-foreground">{day}</span>
                {dayEvents.map((event, eventIndex) => <a key={eventIndex} href={eventUrl(event)} target="_blank" rel="noreferrer" className="mt-1 block break-words rounded border border-t-4 bg-background p-1 text-xs hover:underline" style={{ borderColor: gameColor(event.appId) }} title={`${event.name} · ${playEventLabel(event)}`}>
                  <EventCover src={covers.get(event.appId) ?? event.cover} />
                  <EventLabel event={event} /><span className="block">{event.name}</span>
                </a>)}
              </div>;
            })}
              </div>
              <div className="mt-1 space-y-1">
                {periods.map((period, index) => {
                  const span = periodSpan(period, weekStart, weekEnd, now);
                  if (!span) return null;
                  const leading = Math.max(0, 1 - firstDay);
                  const description = `${period.start.name}: Start ${dateLabel(period.start.at)} — ${period.end ? `${playEventLabel(period.end)} ${dateLabel(period.end.at)}` : 'En curso hasta hoy'}`;
                  return <div key={index} className="grid grid-cols-7">
                    <div className="min-w-0 border-t-2 px-1 text-xs" style={{ gridColumn: `${leading + span.start + 1} / ${leading + span.end + 2}`, borderColor: gameColor(period.start.appId) }} title={description} aria-label={description}>
                      <span className="block truncate">{period.start.name}</span>
                    </div>
                  </div>;
                })}
              </div>
