...
export function LibraryView({
  ...
  const [quickFilter, setQuickFilter] = useState<'all' | 'favorites' | 'playing' | 'pending' | 'shared' | 'short'>('all');
  ...
  const visibleGames = orderedGames.filter((g) => {
    ...
    if (quickFilter === 'playing') return ['playing', 'paused'].includes(pref?.status ?? '');
    if (quickFilter === 'pending') return !pref?.status || pref.status === 'pending';
    if (quickFilter === 'shared') return g.shared;
    if (quickFilter === 'short') {
      const hours = g.hltb?.mainHours ?? g.durationHours;
      return hours !== null && hours !== undefined && hours > 0 && hours <= 5;
    }
    return true;
  });
  ...
  return (
    <div className="hud-library-view">
      ...
      <div className="hud-library-filter-strip">
        ...
        <div className="hud-library-selects">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="">Todas las bibliotecas</SelectItem>
              <SelectItem value="own">Mis juegos propios</SelectItem>
              <SelectItem value="shared">Solo compartidos</SelectItem>
              {(state?.family?.members ?? [])
                .filter((m) => m.steamId !== state?.profile?.steamId)
                .map((m) => (
                  <SelectItem key={m.steamId} value={m.steamId}>
          ...
          <Select
            ...
            items={[
              ...
              { value: 'release-newest', label: 'Lanzamiento: más recientes' },
              { value: 'release-oldest', label: 'Lanzamiento: más antiguos' },
              { value: 'duration-shortest', label: 'Duración: más cortos' },
              { value: 'duration-longest', label: 'Duración: más largos' },
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="original">Orden original</SelectItem>
              <SelectItem value="name">Nombre (A-Z)</SelectItem>
              <SelectItem value="playtime-most">Más horas jugadas</SelectItem>
              <SelectItem value="playtime-least">Menos horas jugadas</SelectItem>
              <SelectItem value="recent-most">Más actividad reciente</SelectItem>
              <SelectItem value="release-newest">Lanzamiento: más recientes</SelectItem>
              <SelectItem value="release-oldest">Lanzamiento: más antiguos</SelectItem>
              <SelectItem value="duration-shortest">Duración: más cortos</SelectItem>
              <SelectItem value="duration-longest">Duración: más largos</SelectItem>
              <SelectItem value="favorite">Favoritos primero</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <div className="hud-library-count-row">
        <span>
          Mostrando <strong>{Math.min(limit, visibleGames.length)}</strong> de{' '}
          <strong>{visibleGames.length}</strong> juegos{' '}
          {visibleGames.length !== games.length && `(de un catálogo de ${games.length})`}
        </span>
      </div>
      ...
      {visibleGames.length === 0 ? (
        ...
        <div className="hud-library-grid">
          {visibleGames.slice(0, limit).map((game) => {
            ...
            return (
              <article key={game.appId} className="hud-library-card">
                {/* Poster Cover */}
                <div className="hud-lib-cover-wrap">
                  ...
                  <div className="hud-lib-overlay-top">
                    <span className="hud-lib-origin-pill">{libraryLabel(game)}</span>
                    <button
                      type="button"
                      className={`hud-lib-fav-btn ${pref.favorite ? 'active' : ''}`}
                      onClick={() => onPreference(game, { favorite: !pref.favorite })}
                      aria-label="Marcar favorito"
                    >
                      <Star size={14} className={pref.favorite ? 'fill-current' : ''} />
                    </button>
                  </div>
                  ...
                  {game.hltb?.mainHours && (
                    <div className="hud-lib-duration-pill">
                      <Clock size={10} className="mr-1 text-emerald-400" />
                      <span>{formatHours(game.hltb.mainHours)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="hud-lib-details">
                  <h4 className="hud-lib-title" title={game.name}>
                    {game.name}
                  </h4>
                  ...
                  <div className="hud-lib-meta-row">
                    ...
                    <span className="hud-status-badge" data-status={pref.status}>
                  ...
                  <div className="hud-lib-actions">
                    <Button
                      ...
                      title={isSaved ? 'Quitar de lista corta' : 'Añadir a lista corta'}
                    >
                    ...
                    >
                      <Sparkles size={11} className="mr-1 text-cyan-400" />
                      Similar
                    </Button>
                  ...
                  <div className="mt-2 pt-2 border-t border-border/30">
                    <Select
                      value={pref.status}
                      onValueChange={(val) =>
                        val && onPreference(game, { status: val as GameStatus })
                      }
                      items={Object.entries(STATUS_LABELS).map(([v, l]) => ({
                        value: v,
                        label: l,
                      }))}
                    >
                  ...
                  <GameOpinion
                    ...
                    preference={pref}
                    busy={!!busy}
                    onSave={(change) => onPreference(game, change)}
                  />
                </div>
              </article>
            );
                  ...
                  <tr key={game.appId}>
                    ...
                    <td>
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="font-semibold text-foreground text-xs">{game.name}</span>
                      </div>
                    </td>
                    <td>
                      <span className="hud-lib-origin-pill text-[10px]">{libraryLabel(game)}</span>
                    </td>
                    ...
                    <td className="text-xs text-muted-foreground">
                      {game.hltb?.mainHours ? `${formatHours(game.hltb.mainHours)}` : '—'}
                    </td>
                    <td>
                      <span className="hud-status-badge" data-status={pref.status}>
                        {STATUS_LABELS[pref.status]}
