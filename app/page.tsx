  ...
  useEffect(() => {
    ...
    const sync = () => {
      void api('steam/achievements/sync', {})
        ...
        .catch((e) => logError('browser', 'achievements:sync-failed', e));
    };
  ...
  async function addGame(input: AddGameInput) {
    ...
    try {
      ...
      setSearch(
        next.games.find(
          (game) =>
            game.igdbId === input.igdbId && game.platform === input.platform,
        )?.name ?? '',
      );
      setLibraryOwner('');
      setLibraryTag('');
                    ...
                    <article key={game.appId} className="hud-game-card">
                      <div className="hud-card-cover-wrapper">
                        ...
                        <div className="hud-card-overlay-badges">
                          ...
                          <span
                            ...
                            data-status={state?.preferences[game.appId]?.status}
                          >
                            {
                              STATUS_LABELS[
                                state?.preferences[game.appId]?.status ??
                      ...
                      <div className="hud-card-body">
                        ...
                        <div className="hud-card-actions">
                          ...
                          >
                            <span>
                              {game.appId > 0 ? 'Abrir Steam' : 'Ver en IGDB'}
                            </span>
            ...
            <div className="hud-shelf-section space-y-4">
              ...
              {!savedGames.length ? (
                <div className="empty-state">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Guarda juegos con el icono de marcador desde "Para ti" o "Tu
                    biblioteca".
                  </p>
                </div>
              ) : (
                    ...
                    <article key={game.appId} className="hud-game-card">
                      ...
                      <div className="hud-card-body">
                        ...
                        <div className="text-xs text-muted-foreground space-y-1 mb-2">
                          {game.hltb?.mainHours && (
                            <div>
                              HLTB: {formatHours(game.hltb.mainHours)} de
                              historia
                            </div>
                          )}
                          {game.durationHours && (
          ...
          <TabsContent value="history">
            <div className="results space-y-4">
              <p className="small-note">
                ...
                incluso después de empezar otra búsqueda.
              </p>
              ...
              {state?.history?.toReversed().map((entry) => (
                <details className="panel saved-search" key={entry.id}>
                  ...
                  <div className="hud-bento-grid mt-4">
                    {[...entry.result.owned, ...entry.result.discoveries].map(
                      (pick) => (
                        <GameCard
                          key={pick.appId}
                          pick={pick}
