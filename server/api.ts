...
export async function handle(
    ...
    try {
        ...
        if (path === '/api/igdb/video' && request.method === 'GET') {
            ...
            if (path === '/api/state' && request.method === 'PATCH') {
                ...
                if (
                    ...
                    (!state.games.some((g) => g.appId === payload.appId) &&
                        ![...state.conversation, ...(state.history ?? [])].some(
                            (turn) =>
                                [
                                    ...turn.result.owned,
            ...
            if (path === '/api/steam/tags/sync' && request.method === 'POST') {
                ...
                try {
                    result = await syncSteamTags({
                        ...
                        onLog: (message) =>
                            phase('steam:tags:sync:log', { message }),
                    });
                } catch (error) {
            ...
            if (path === '/api/steam/sync' && request.method === 'POST') {
                ...
                let next: State = {
                    ...
                    games: mergeLibraries(
                        [
                            ...fresh.games,
                            ...state.games.filter((game) => game.appId < 0),
                        ],
            ...
            if (path === '/api/recommendations' && request.method === 'POST') {
                ...
                if (state.syncedAt && Date.now() - state.syncedAt >= DAY) {
                    ...
                    try {
                        ...
                        logError(
                            'server',
                            'recommendations:library-refresh:error',
                            e,
                            {
                                path,
                            },
                ...
                if (
                    ...
                    try {
                        ...
                        logError(
                            'server',
                            'recommendations:family-refresh:error',
                            e,
                            {
                                path,
                            },
                ...
                if (!filters.shortlistOnly && c.clientId && c.clientSecret) {
                    ...
                    try {
                        ...
                        logError(
                            'server',
                            'recommendations:discover:error',
                            e,
                            { path },
                        );
                        warnings.push(
                ...
                phase('recommendations:complete', {
                    owned:
                        nextSnapshot.conversation.at(-1)?.result.owned.length ??
                        0,
                    discoveries:
                        nextSnapshot.conversation.at(-1)?.result.discoveries
                            .length ?? 0,
