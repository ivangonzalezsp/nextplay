...
export function parseFilters(value: unknown): Filters {
    ...
    const tags = f.tags ?? [];
    const sessionIntent =
        f.sessionIntent === undefined ? 'any' : f.sessionIntent;
    ...
    if (
        (f.shortlistOnly !== undefined &&
            typeof f.shortlistOnly !== 'boolean') ||
...
export function parsePreference(value: unknown): Preference {
    ...
    return {
        ...
        ...(p.opinion
            ? {
                  opinion: p.opinion,
                  opinionReason: p.opinionReason?.trim() ?? '',
              }
            : {}),
    };
}
...
export function recentRecommendationPenalties(state: State) {
    ...
    for (const [index, turn] of recent.entries())
        for (const pick of [...turn.result.owned, ...turn.result.discoveries])
            if (!penalties.has(pick.appId))
                penalties.set(pick.appId, 30 - index * 6);
    return penalties;
}
...
export function selectCandidates(
    ...
    return filterCandidates(
        [...unique.values()]
            ...
            .toSorted(
                (a, b) => b.score - a.score || a.game.appId - b.game.appId,
            )
            .map(({ game }) => game),
        state.preferences,
        filters,
