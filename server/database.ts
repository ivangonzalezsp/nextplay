...
export function loadState(db: DatabaseSync): State | undefined {
  ...
  try {
    ...
    return {
      ...
      games: overlaySteamAchievements(db, overlaySteamTags(db, games)),
    };
  } finally {
    db.exec('COMMIT');
  }
...
function validSteamTag(value: unknown): value is SteamTag {
  ...
  return (
    ...
    (tag.englishName === undefined ||
      (typeof tag.englishName === 'string' && tag.englishName.length <= 200))
  );
}
...
function validAchievements(
  value: unknown,
): value is NonNullable<Game['steamAchievements']> {
  if (!value || typeof value !== 'object') return false;
  const data = value as Record<string, unknown>;
  return (
    Number.isSafeInteger(data.unlocked) &&
    Number(data.unlocked) >= 0 &&
    Number.isSafeInteger(data.total) &&
    Number(data.total) >= 0 &&
    Number(data.unlocked) <= Number(data.total) &&
    Number.isSafeInteger(data.at) &&
    Number(data.at) > 0 &&
    Array.isArray(data.achievements) &&
    data.achievements.length === Number(data.total)
  );
}

export function overlaySteamAchievements<T extends Game>(
  db: DatabaseSync,
  games: T[],
): T[] {
  let rows: { app_id: unknown; data: unknown }[];
  try {
    rows = db.prepare('SELECT app_id, data FROM steam_achievements').all() as {
      app_id: unknown;
      data: unknown;
    }[];
  } catch {
  ...
  const records = new Map<number, NonNullable<Game['steamAchievements']>>();
  for (const row of rows) {
    try {
      const appId = Number(row.app_id);
      const data = JSON.parse(String(row.data));
      if (Number.isSafeInteger(appId) && appId > 0 && validAchievements(data))
        records.set(appId, data);
    } catch {}
  }
  return games.map((game) => {
    const data = records.get(game.appId);
    return data ? ({ ...game, steamAchievements: data } as T) : game;
  });
}

export function writeSteamAchievements(
  db: DatabaseSync,
  appId: number,
  data: NonNullable<Game['steamAchievements']>,
) {
  if (!Number.isSafeInteger(appId) || appId <= 0 || !validAchievements(data))
    throw new Error('Invalid Steam achievements write.');
  db.prepare(
    `INSERT INTO steam_achievements(app_id, data) VALUES (?, ?)
         ON CONFLICT(app_id) DO UPDATE SET data = excluded.data`,
  ).run(appId, JSON.stringify(data));
}
    ...
    for (const write of writes) {
      if (
        ...
        (write.checkedAt !== null &&
          (!Number.isSafeInteger(write.checkedAt) || write.checkedAt <= 0))
      )
