...
export async function runProcess(
    ...
    return new Promise<string>((resolveRun, reject) => {
        ...
        child.on('close', (code) => {
            ...
            const specific = specificErrors.find(([pattern]) =>
                pattern.test(msg),
            );
...
DATOS_JSON:\n` +
        JSON.stringify({
            ...
            preferences: Object.entries(state.preferences).map(
                ([appId, preference]) => ({
                    appId: Number(appId),
                    name: names.get(Number(appId)) ?? `Steam ${appId}`,
                    ...preference,
                }),
            ),
            history: state.conversation.map((t) => ({
                ...
                picks: [...t.result.owned, ...t.result.discoveries].map(
                    (p) => ({
                        appId: p.appId,
                        name: p.game.name,
                        reason: p.reason,
    ...
    try {
        ...
        if (catalog) {
            ...
            try {
                ...
                replaceGames(
                    db,
                    catalog.candidates.map((game) =>
                        catalogGame(game, catalog.state),
                    ),
                );
                db.exec('COMMIT');
            } finally {
        ...
        if (
            dir.startsWith(
                runRoot + (process.platform === 'win32' ? '\\' : '/'),
            )
        )
