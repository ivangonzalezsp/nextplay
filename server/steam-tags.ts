...
export function createSteamTagClient(options: SteamTagClientOptions = {}) {
  ...
  async function requestText(url: string) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (response.url) {
        try {
          ...
          if (
            ...
            throw new SteamTagsError('Steam redirigió la consulta.', {
              retryable: false,
            });
        } catch (error) {
          if (error instanceof SteamTagsError) throw error;
          throw new SteamTagsError('Steam devolvió una URL inválida.');
        }
      }
      ...
      try {
        ...
        throw new SteamTagsError('Steam devolvió una respuesta ilegible.');
      }
    }
  ...
  return {
    async loadDictionaries() {
      const english = JSON.parse(await requestText(tagDataUrl('english')));
      const spanish = JSON.parse(await requestText(tagDataUrl('spanish')));
