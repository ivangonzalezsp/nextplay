...
export function ownedGames(response: unknown): Game[] {
  ...
  const games: Game[] = data.games.map((g) => {
    if (
      ...
      throw new AppError('Steam devolvió un juego con datos incompletos.', 502);
    return {
      appId: g.appid,
      name: g.name.slice(0, 300),
      owned: true,
      playtimeMinutes: natural(g.playtime_forever) ? g.playtime_forever : null,
      recentMinutes: null,
      cover:
  ...
  try {
    ...
    if (Array.isArray(recent))
      for (const game of games) {
        ...
        game.recentMinutes =
          row && natural(row.playtime_2weeks) ? row.playtime_2weeks : 0;
      }
...
async function igdb<T = IgdbGame>(
  ...
  if (
    ...
    if (typeof t.access_token !== 'string' || !natural(t.expires_in))
      throw new AppError('Twitch no devolvió un acceso válido a IGDB.', 502);
    token = {
...
let dictionaries:
  | { source: number; gameTypes: Set<number>; until: number }
  | undefined;
...
export function metadata(
  ...
  return {
    ...
    summary:
      typeof raw.summary === 'string' ? raw.summary.slice(0, 1500) : undefined,
    ...(cover ? { cover } : {}),
