...
export function validateHltb(value: unknown, games: Game[]) {
  ...
  return value.map((row) => {
    if (
      ...
      throw new AppError('Identificador no válido de HowLongToBeat.', 502);
    used.add(row.appId);
    const d = row.data;
    if (
      d !== null &&
    ...
    const child = execFile(
      ...
      (error, stdout) => {
        ...
        try {
          ...
          reject(new AppError('HowLongToBeat devolvió datos ilegibles.', 502));
        }
      },
    );
    child.stdin?.on('error', () => {});
    child.stdin?.end(
      JSON.stringify(games.map((g) => ({ appId: g.appId, name: g.name }))),
    );
  });
}
